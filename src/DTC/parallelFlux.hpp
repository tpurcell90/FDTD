        ...
        freqList_(freqList)
    {
        ...
        if(!cross_sec)
        {
            if(propDir == DIRECTION::Y)
                ...
                throw std::logic_error("Need a direction of propagation to correctly normalize flux" );
        }
...
public:
    parallelFluxDTCReal(std::shared_ptr<mpiInterface> gridComm, std::string name, double weight, pgrid_ptr Ex, pgrid_ptr Ey, pgrid_ptr Ez, pgrid_ptr Hx, pgrid_ptr Hy, pgrid_ptr Hz, std::array<int,3> loc, std::array<int,3> sz, bool cross_sec, bool save, bool load, int timeInt, std::vector<double> freqList, DIRECTION propDir, std::array<double,3> d, double dt, double theta, double phi, double psi, double alpha, std::string incd_file, bool SI, double I0, double a);
    // void fieldIn(double& tt);
