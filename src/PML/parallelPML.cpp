...
void pmlUpdateFxnCplx::addPsi(std::vector<updateGridParams> &gridParamList, std::vector<updatePsiParams> &psiParamList, cplx_pgrid_ptr grid_i, cplx_pgrid_ptr psi, cplx_pgrid_ptr grid)
{
    updatePsiField(psiParamList, psi, grid);
    for(auto & param : gridParamList)
        zaxpy_(param.nAx_, param.Db_, &psi->point(param.loc_[0], param.loc_[1], 0), param.stride_, &grid_i->point(param.loc_[0], param.loc_[1], 0), param.stride_);
}
...
parallelCPMLReal::parallelCPMLReal(mpiInterface gridComm, std::vector<real_grid_ptr> weights, real_pgrid_ptr grid_i, real_pgrid_ptr grid_j, real_pgrid_ptr grid_k, POLARIZATION pol_i, std::array<int,3> n_vec, double m, double ma, double aMax, std::array<double,3> d, double dt, int_pgrid_ptr phys_Ex, int_pgrid_ptr phys_Ey, std::vector<std::shared_ptr<Obj>> objArr) :
    parallelCPML<double>(gridComm, weights, grid_i, grid_j, grid_k, pol_i, n_vec, m, ma, aMax, d, dt, phys_Ex, phys_Ey, objArr)
{
    if(psi_j_)
...
parallelCPMLCplx::parallelCPMLCplx(mpiInterface gridComm, std::vector<real_grid_ptr> weights, std::shared_ptr<parallelGrid<cplx > > grid_i, std::shared_ptr<parallelGrid<cplx > > grid_j, std::shared_ptr<parallelGrid<cplx > > grid_k, POLARIZATION pol_i, std::array<int,3> n_vec, double m, double ma, double aMax, std::array<double,3> d, double dt, int_pgrid_ptr phys_Ex, int_pgrid_ptr phys_Ey, std::vector<std::shared_ptr<Obj>> objArr) :
    parallelCPML<cplx>(gridComm, weights, grid_i, grid_j, grid_k, pol_i, n_vec, m, ma, aMax, d, dt, phys_Ex, phys_Ey, objArr)
{
    if(psi_j_)
