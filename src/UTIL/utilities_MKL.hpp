...
extern "C"
{
    ...
    void zgerc_ (const int* m, const int* n, const cplx *alpha, const cplx *x, const int* incx, const cplx *y, const int* incy, cplx *a, const int* lda);

    void zgemm3m_(char* transa, char* transb, const int* m, const int* n, const int* k, const cplx* alpha, const cplx* a, const int* lda, const cplx* b, const int* ldb, const cplx* beta, cplx* c, const int* ldc);

    void zhemm_(char* side, char* uplo, const int* m, const int* n, const cplx *alpha, const cplx *a, const int* lda, const cplx *b, const int* ldb, const cplx *beta, const cplx *c, const int* ldc);
    ...
    void mkl_zomatadd (const char*, const char*, const char*, const int* m, const int* n, const cplx* alpha, const cplx* A, const int* lda, const cplx* beta, const cplx* B, const int* ldb, cplx* C, const int* ldc);
...
namespace
{
     ...
     * @param[in]  ldc     Specifies the leading dimension of c as declared in the calling (sub)program.
     */
    void dgemm_(char transa, char transb, const int m, const int n, const int k, const double alpha, const std::unique_ptr<double []>& a, const int lda, const std::unique_ptr<double []>& b, const int ldb, const double beta, std::unique_ptr<double []>& c, const int ldc)
   ...
   /**
    ...
    /**
     * @brief      wrapper for zgerc_
     *
     * @param[in]  m      Specifies the number of rows of the matrix A.
     * @param[in]  n      Specifies the number of columns of the matrix A.
     * @param[in]  alpha  Specifies the scalar alpha.
     * @param[in]  x      Array, size at least (1 + (m - 1)*abs(incx)). Before entry, the incremented array x must contain the m-element vector x.
     * @param[in]  incx   Specifies the increment for the elements of x.
     * @param[in]  y      Array, size at least (1 + (n - 1)*abs(incy)). Before entry, the incremented array y must contain the n-element vector y.
     * @param[in]  incy   Specifies the increment for the elements of y.
     * @param      a      Array, size lda*k. k is n. Before entry, the leading m-by-n part of the array a must contain the matrix A.
     * @param[in]  lda    Specifies the leading dimension of a as declared in the calling (sub)program. The value of lda must be at least max(1, m)
     */
    void zgerc_ (const int m, const int n, const cplx alpha, const cplx *x, const int incx, const cplx *y, const int incy, cplx *a, const int lda)
     ...
     * @param[in]  transb  'T' or 't' for Transpose, 'C' or c' for Hermitian Conjugate, 'N' or 'n' for neither on Matrix B
     * @param[in]  m       Specifies the number of rows of the matrix op(A) and of the matrix C. The value of m must be at least zero.
     * @param[in]  n       Specifies the number of columns of the matrix op(B) and the number of columns of the matrix C.
     * @param[in]  k       Specifies the number of columns of the matrix op(A) and the number of rows of the matrix op(B).
     * @param[in]  alpha   Specifies the scalar alpha.
     * @param[in]  a       Array size lda by ka, where ka is k when transa = 'N' or 'n', and is m otherwise. Before entry with transa = 'N' or 'n', the leading m-by-k part of the array a must contain the matrix A, otherwise the leading k-by-m part of the array a must contain the matrix A.
     * @param[in]  lda     specifies the leading dimension of a as declared in the calling (sub)program.
     ...
     * @param[in]  ldc     Specifies the leading dimension of c as declared in the calling (sub)program.
     */
    void zgemm_(char transa, char transb, const int& m, const int& n, const int& k, const cplx& alpha, const cplx* a, const int& lda, const cplx* b, const int& ldb, const cplx& beta, cplx* c, const int& ldc)
     ...
     * @param[in]  ldc     Specifies the leading dimension of c as declared in the calling (sub)program.
     */
    void zgemm_(char transa, char transb, const int& m, const int& n, const int& k, const cplx& alpha, const std::unique_ptr<cplx[]>& a, const int& lda, const std::unique_ptr<cplx[]>& b, const int& ldb, const cplx& beta, std::unique_ptr<cplx[]>& c, const int& ldc)
    ...
    void zhemm_(char side, char uplo, const int m, const int n, const cplx alpha, const std::unique_ptr<cplx[]>& a, const int lda, const std::unique_ptr<cplx[]>& b, const int ldb, const cplx beta, const std::unique_ptr<cplx[]>& c, const int ldc)
     ...
     *
     * @param[in]  ordering  Ordering of the matrix storage. If ordering = 'R' or 'r', the ordering is row-major. If ordering = 'C' or 'c', the ordering is column-major.
     * @param[in]  transa    Parameter that specifies the operation type. If trans = 'N' or 'n', op(A)=A and the matrix A is assumed unchanged on input. If trans = 'T' or 't', it is assumed that A should be transposed. If trans = 'C' or 'c', it is assumed that A should be conjugate transposed. If trans = 'R' or 'r', it is assumed that A should be only conjugated. If the data is real, then trans = 'R' is the same as trans = 'N', and trans = 'C' is the same as trans = 'T'.
     * @param[in]  transb    Parameter that specifies the operation type. If trans = 'N' or 'n', op(B)=B and the matrix B is assumed unchanged on input. If trans = 'T' or 't', it is assumed that A should be transposed. If trans = 'C' or 'c', it is assumed that A should be conjugate transposed. If trans = 'R' or 'r', it is assumed that A should be only conjugated. If the data is real, then trans = 'R' is the same as trans = 'N', and trans = 'C' is the same as trans = 'T'.
     * @param[in]  m         The number of matrix rows in op(A), op(B), and C.
     * @param[in]  n         The number of matrix columns in op(A), op(B), and C.
     * @param[in]  alpha     This parameter scales the input matrix op(A) by alpha.
     * @param[in]  A         Array
     * @param[in]  lda       Distance between the first elements in adjacent columns (in the case of the column-major order) or rows (in the case of the row-major order) in the source matrix A; measured in the number of elements. For ordering = 'C' or 'c': when transa = 'N', 'n', 'R', or 'r', lda must be at least max(1,m); otherwise lda must be max(1,n). For ordering = 'R' or 'r': when transa = 'N', 'n', 'R', or 'r', lda must be at least max(1,n); otherwise lda must be max(1,m).
     * @param[in]  beta      This parameter scales the input matrix op(B) by alpha.
     * @param[in]  B         Array
     * @param[in]  ldb       Distance between the first elements in adjacent columns (in the case of the column-major order) or rows (in the case of the row-major order) in the source matrix B; measured in the number of elements. For ordering = 'C' or 'c': when transa = 'N', 'n', 'R', or 'r', ldb must be at least max(1,m); otherwise ldb must be max(1,n). For ordering = 'R' or 'r': when transa = 'N', 'n', 'R', or 'r', ldb must be at least max(1,n); otherwise ldb must be max(1,m).
     * @param[in]  C         Array
     ...
     */
    void mkl_zomatadd_(char ordering, char transa, char transb, const int m, const int n, const cplx alpha, const cplx* A, const int lda, const cplx beta, const cplx* B, const int ldb, cplx* C, const int ldc)
